...
if __name__ == '__main__':
    ...
    validation_image_path = "wsss4luad_valid_out_cam/crop_images/"
    inter_union = np.zeros((2, 3))

    infer_dataset = voc12.data.MyImageDataset(
    ...
    inter_union = np.zeros((2,3))
    for name, img, _ in tqdm(infer_data_loader):
        name = name[0].split('.')[0]
        ...
        with torch.no_grad():
            ...
            gt = np.asarray(Image.open(f'wsss4luad_valid_out_cam/gt/{name}.png'))
            
            for i in range(3):
                if i in res:
                    inter = np.sum(np.logical_and(res == i, gt == i))
                    u = np.sum(np.logical_or(res == i, gt == i))
                    inter_union[0,i] += inter
                    inter_union[1,i] += u
            
            np.save(os.path.join(args.out_rw, name + '.npy'), res)
